...
The result must be an Android-first, offline-first mobile client for the
existing RCH ecosystem, not a replacement for the Python hub.
...
target is no longer just a generic mobile Reticulum node. The target is a full
**client-side RCH implementation** with R3AKT support and these constraints:

- The OpenAPI file is **not** a REST implementation target.
- `Reticulum-Telemetry-Hub/API/ReticulumCommunityHub-OAS.yaml` is only a
  description of operation intent and payload shapes.
...
Maintain the canonical message catalog:
...
Use `docs/R3AKTClient/APIANnalysis_clientImplementationSet.md` as the implementation
...
Use `docs/R3AKTClient/APIANnalysis_clientImplementationSet.md` as the allowed feature
surface.

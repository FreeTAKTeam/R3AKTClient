...
Source: `C:\Users\broth\Documents\work\ATAK\src\Reticulum-Telemetry-Hub\API\ReticulumCommunityHub-OAS.yaml`

This file is an operation inventory for the Android-first RCH client.
The mobile app does not use these REST endpoints as its runtime feature API;
this list is used as the compatibility reference for which RCH capabilities
must be exposed through the local Rust Reticulum/LXMF client runtime.
...
| `GET /Examples` | Return command descriptions and JSON payload examples. | `client` |
...
| `GET /Identities` | List identity moderation status entries. | `server-only` |
...
| `GET /Reticulum/Config` | Return the raw Reticulum config content. | `server-only` |
| `PUT /Reticulum/Config` | Apply a new Reticulum config payload. | `server-only` |
| `POST /Reticulum/Config/Rollback` | Roll back Reticulum config using a backup. | `server-only` |
| `POST /Reticulum/Config/Validate` | Validate a Reticulum config payload without applying. | `server-only` |
| `GET /Reticulum/Discovery` | Return a live snapshot of Reticulum interface discovery state. | `server-only` |
| `GET /Reticulum/Interfaces/Capabilities` | Return Reticulum runtime interface capabilities. | `server-only` |
...
| `POST /Topic/Associate` | Associate an attachment upload with a TopicID. | `server-only` |

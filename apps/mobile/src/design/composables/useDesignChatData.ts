...
function systemLabel(topicId?: string, destination?: string): string {
  if (topicId) {
...
export function useDesignChatData() {
  ...
  async function wireStores() {
    ...
    errorMessage.value = "";
    await messagingStore.wire().catch((error: unknown) => {
  ...
  watch(
    ...
    (channels) => {
      if (messagingStore.activeMessages.length > 0 || channels.length === 0) {
        return;
      }
      const richestChannel = channels.find((channel) => channel.messageIds.length > 0);
      if (richestChannel) {
        messagingStore.setActiveChannel(richestChannel.channelKey);
      }
    },
    { immediate: true },
  );
  ...
  const messages = computed<ChatMessageModel[]>(() =>
    messagingStore.activeMessages.map((message) => ({
      ...
      label:
        message.direction === "outbound"
          ? "Operative 01"
          : systemLabel(message.topicId, message.destination),
  ...
  const timestampLabel = computed(() => {
    ...
    const sourceTimestamp = latest?.issuedAt ?? new Date().toISOString();
    return `Today ${formatTimestamp(sourceTimestamp)} UTC`;
  });

  const liveLabel = computed(() =>
    messagingStore.ready ? "Encrypted Connection" : "Bridge Offline",
  );

  async function sendCurrentDraft(text?: string) {
    ...
    await messagingStore.sendDraft().catch((error: unknown) => {
      errorMessage.value = toErrorMessage(error);
    });
  }

...
export function useDesignChecklistsData() {
  ...
  const taskOverrides = reactive<Record<string, boolean>>({});
  ...
  watch(
    ...
    (checklistId) => {
      ...
      if (!checklist || checklist.tasks.length === 0) {
        checklistsStore.getChecklist(checklistId).catch((error: unknown) => {
          errorMessage.value = toErrorMessage(error);
        });
      }
    },
  ...
  const selectedChecklist = computed(() => {
    const checklist =
      checklistsStore.checklistsById[selectedChecklistId.value]
      ?? checklists.value[0];
    if (!checklist) {
      return undefined;
    }
    ...
    return {
      ...
      tasks: checklist.tasks.map((task) => ({
        ...task,
        isComplete:
          taskOverrides[`${checklist.checklistId}:${task.taskId}`] ?? task.isComplete,
      })),
    };
  });
  ...
  function progressFor(checklistId: string): number {
    const checklist = checklistsStore.checklistsById[checklistId];
    if (!checklist || checklist.tasks.length === 0) {
      return 0;
    }
  ...
  function statusLabel(complete: boolean): string {
  ...
  async function toggleTask(taskId: string) {
    ...
    try {
      await checklistsStore.setTaskStatus(
        selectedChecklist.value.checklistId,
        taskId,

...
interface MissionTimelineModel {
  id: string;
  time: string;
  tone: "muted" | "primary";
  value: string;
...
interface MissionDirectoryItemModel {
  id: string;
  missionId: string;
  status: string;
  subtitle: string;
...
function priorityTone(priority?: number): "high" | "medium" | "low" {
  if ((priority ?? 0) >= 7) {
    return "high";
  }
  if ((priority ?? 0) >= 4) {
...
function actionStyle(status?: string): "ghost" | "primary" {
  return status?.toLowerCase().includes("active") ? "primary" : "ghost";
}

function buildMissionSubtitle(description?: string, path?: string): string {
  if (description) {
    return description;
  }
  if (path) {
...
export function useDesignMissionsData() {
  ...
  async function wireStores() {
    ...
    errorMessage.value = "";
    await missionCoreStore.wire().catch((error: unknown) => {
  ...
  const liveDirectory = computed<MissionDirectoryItemModel[]>(() =>
    missionCoreStore.missions.map((mission) => {
      const timeline: MissionTimelineModel[] = missionCoreStore.missionLogEntries
  ...
  const selectedMission = computed(
    () =>
      missionDirectory.value.find((mission) => mission.id === selectedMissionId.value)
      ?? missionDirectory.value[0],
  );

  const missionCards = computed<MissionCardModel[]>(() =>
    missionCoreStore.missions.map((mission, index) => ({
      actionIcon:
        actionLabel(mission.missionStatus) === "Monitor"
          ? "visibility"
      ...
      delta:
        mission.missionPriority !== undefined
      ...
      metricLabel: "Logs",
      metricValue: `${
        missionCoreStore.missionLogEntries.filter((entry) => entry.missionUid === mission.uid).length
      }`,
  ...
  const statCards = computed<MissionStatCard[]>(() => {

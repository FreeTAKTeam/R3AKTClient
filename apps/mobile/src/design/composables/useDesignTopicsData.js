...
function branchIdFromTopic(topicId, topicPath) {
...
export function useDesignTopicsData() {
    ...
    const topicItems = computed(() => topicsStore.topics.map((topic) => {
    ...
    async function addTopic() {
        const topicId = searchTerm.value.trim()
        ...
        await topicsStore.subscribeTopic(topicId).catch((error) => {
            errorMessage.value = toErrorMessage(error);
        });
        searchTerm.value = "";
    }

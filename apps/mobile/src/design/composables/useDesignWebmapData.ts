...
function formatCoordinate(value: number | undefined, axis: "lat" | "lon"): string {
...
export function useDesignWebmapData() {
  ...
  const activeTool = shallowRef<"marker" | "zone">("marker");
  ...
  async function wireStore() {
    ...
    errorMessage.value = "";
    await mapStore.wire().catch((error: unknown) => {
  ...
  function selectTool(tool: "marker" | "zone") {
  ...
  async function recenterMap() {
    if (activeTool.value === "zone" && mapStore.zones.length > 0) {
      ...
      await mapStore.listZones().catch((error: unknown) => {
        errorMessage.value = toErrorMessage(error);
      });
      return;
    }
    ...
    if (mapStore.markers.length > 0) {
      ...
      await mapStore.listMarkers().catch((error: unknown) => {
        errorMessage.value = toErrorMessage(error);
      });
    }
  }

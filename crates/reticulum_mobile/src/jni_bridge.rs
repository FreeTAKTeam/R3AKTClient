...
#[no_mangle]
pub extern "system" fn Java_network_reticulum_emergency_ReticulumBridge_executeEnvelope(
    mut env: JNIEnv,
    _class: JClass,
    envelope_json: JString,
) -> jstring {
    let envelope = match jstring_to_rust(&mut env, envelope_json) {
        Ok(v) => v,
        Err(e) => {
    ...
    match node.execute_envelope(envelope) {

...
fn is_allowed_operation(operation: &str) -> bool {
    CLIENT_OPERATION_KEYS.iter().any(|entry| *entry == operation)
}

fn build_mission_sync_command(envelope: &MessageEnvelope, source_identity: &str) -> Result<String, NodeError> {
...
fn collect_lxmf_text(reply: &LxmfMessage) -> String {
...
async fn execute_envelope_over_lxmf(
    ...
    let content = build_mission_sync_command(envelope, &source_identity)?;

    let mut message = LxmfMessage::new();
    message.source_hash = Some(source);
    message.destination_hash = Some(destination);
    message.set_title_from_string(&envelope.r#type);
    ...
    loop {
        ...
        let text = collect_lxmf_text(&reply);
        if !text.contains(&envelope.message_id) && !text.contains(&correlation) {
            continue;
        }
        ...
        return serde_json::to_string(&result).map_err(|_| NodeError::InternalError {});
    }

    let timeout_result = MessageEnvelope {
        api_version: envelope.api_version.clone(),
        message_id: envelope.message_id.clone(),
    ...
    serde_json::to_string(&timeout_result).map_err(|_| NodeError::InternalError {})
}
            ...
            Command::ExecuteEnvelope {
                ...
                let result = async {
                    let parsed: MessageEnvelope =
                        serde_json::from_str(&envelope_json).map_err(|_| NodeError::InvalidConfig {})?;
...
mod tests {
    ...
    #[test]
    fn build_mission_sync_command_embeds_correlation_and_type() {
        let envelope = MessageEnvelope {
            api_version: "1.0".to_string(),
        ...
        let command = build_mission_sync_command(&envelope, "source-identity")
            .expect("mission sync command should build");

        assert!(command.contains("corr-3"));
        assert!(command.contains("msg-3"));
        assert!(command.contains("source-identity"));
    }

    #[test]
    fn collect_lxmf_text_includes_title_and_content() {
        let mut message = LxmfMessage::new();
